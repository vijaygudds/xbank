...
class Controller_Sms extends AbstractController{
	...
	function sendMessage($no,$msg,$temp){
		...
		$url="http://smsuser.dsadv.in/http-api.php?username=".$this->app->getConfig('user')."&password=".$password."&senderid=".$this->app->getConfig('senderId')."&route=5&number=".$no."&message=".$msg."&".$temp;		
// echo $url;
		return $curl->get($url);
	}
}
