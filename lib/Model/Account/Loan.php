...
class Model_Account_Loan extends Model_Account{
	...
	function createNewPendingAccount($member_id,$scheme_id,$branch, $AccountNumber,$otherValues=null,$form=null,$created_at=null){
		...
		foreach ($otherValues as $field => $value) {
			...
			// echo $pending_account[$field] = $value . "</br>";
		}
	...
	function createProcessingFeeAndSMTransaction($otherValues, $on_date){
		...
		// throw new \Exception($otherValues['account_type'], 1);
		
	/*this code is temporary not use start*/
		if($otherValues['account_type'] == 'Loan Against Deposit'){
			if($sm_amount){
				$account = $this->add('Model_Account_SM')->addCondition('member_id',$this['member_id'])->setLimit(1)->tryLoadAny();			
				$narration .= " / & Amount Deposited in SM Account " . $account['AccountNumber'];
			} 
	/*this code is temporary not use End*/
	
		/*this code is temporary start*/
		}else{
		// throw new \Exception($this->api->currentBranch['Code']." "."INTEREST RECEIVED ON LOAN", 1);
			...
			if($sm_amount){
				...
				if(!$account->loaded()){
					throw new \Exception($this->api->currentBranch['Code']." INTEREST RECEIVED ON LOAN NOT LOADED");
				}			
			}

				
		/*this code is temporary END*/
		}
	...
	function payPremiums($amount,$on_date){
		...
		$tr_gst_charges= $this->add('Model_TransactionRow')->addCondition('transaction_type',[
								'Visit Charge',
								...
								])->addCondition('account_id',$this->id)->sum('amountCr')->getOne();
		// Access amount then loan amount per premium is actually interest

		// throw new \Exception("Cr Amount ". $this['CurrentBalanceCr']. " GST Charge".$tr_gst_charges . "Other Charges" . $tran_penalty_and_other_amt_received, 1);
		...
		$AmountForPremiums = ($AmountForPremiums- $tr_gst_charges);
