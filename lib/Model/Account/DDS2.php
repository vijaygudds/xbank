<?php
class Model_Account_DDS2 extends Model_Account{
	...
	function init(){
		...
		$this->getElement('Amount')->caption('Monthly Deposit Amount');
		...
		// $this->addCondition('dds_type','DDS2');
	...
	function interestPaid($as_on_date=null){
		...
		$transactions->addCondition('transaction_type_name',TRA_INTEREST_POSTING_IN_DDS);
	...
	function payInterest($on_date=null){
		...
		$transaction->createNewTransaction(TRA_INTEREST_POSTING_IN_DDS, $this->ref('branch_id'), $on_date, 'Interest posting in DDS Account', null, array('reference_id'=>$this->id));
	...
	function pre_mature($on_date=null,$return_amount = false,$account_to_credit=null ,$other_charges=[], $other_bonus=[],$percentages=null,$is_maturity=false){
		...
		if($difference > 0) {
			...
			$transaction = $this->add('Model_Transaction');
			$transaction->createNewTransaction(TRA_INTEREST_POSTING_IN_DDS, $this->ref('branch_id'), $on_date, 'Pre mature remaining interest posting till date in '. $this['AccountNumber'], $only_transaction=null, array('reference_id'=>$this->id));
