...
class page_reports_general_periodicalnew extends Page {
	...
	function page_index(){
		...
		$form->addField('DropDown','status')->setEmptyText('All')->setValueList(array('Active'=>'Active','Inactive'=>'Inactive'));
		...
		$this->add('VirtualPage')
			...
			->set(function($p)use ($status){
				...
				$account_model->addExpression('cr_sum')->set(function($m,$q){
					return $q->expr('IFNULL([0],0) + IFNULL([1],0)',[$m->refSQL('TransactionRow')->sum('amountCr'),$m->getElement('OpeningBalanceCr')]);
				});

				$account_model->addExpression('dr_sum')->set(function($m,$q){
					return $q->expr('IFNULL([0],0) + IFNULL([1],0)',[$m->refSQL('TransactionRow')->sum('amountDr'),$m->getElement('OpeningBalanceDr')]);
				});

				$account_model->addExpression('balance')->set(function($m,$q){
					return $q->expr('IFNULL([0],0) - IFNULL([1],0)',[$m->getElement('cr_sum'),$m->getElement('dr_sum')]);
				});
				...
				$grid->addSno();
				$grid->setModel($account_model,array('member_no','ActiveStatus','sm_no','created_at','maturity_date','balance','AccountNumber','scheme','Amount','pan_no','adhaar_no','member','father_name','address','phone_no','agent','dealer','mo','team','agent_saving_acc','agent_phone_no','Nominee','NomineeAge','RelationWithNominee','repayment_mode','is_in_legal','is_given_for_legal_process','is_in_arbitration'));
