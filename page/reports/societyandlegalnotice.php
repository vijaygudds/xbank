...
class page_reports_societyandlegalnotice extends Page {

	public $title="Society & Legal Notice";
	...
	function page_index(){
		...
		$form->addField('dropdown','print_document')->setValueList(array('society_notice'=>'Society Notice','legal_notice'=>'Legal Notice'));
		// $form->addField('dropdown','report_type')->setValueList(array('one_month'=>'1 Month','two_month'=>'2 Month Due','three_month'=>'3 Month Due','four_month'=>'4 Month Due','five_month'=>'5 Month Due','five_above'=>'5 Month Above Due'));
		...
		$account_model->addExpression('recent_due_premium_date')->set(function($m,$q)use($till_date){
			return $m->refSQL('Premium')
						->addCondition('PaidOn',null)
						->addCondition('DueDate','<',$m->api->nextDate($till_date))
						->setLimit(1)
		...
		if($_GET['filter']){
			...
			$this->api->stickyGET('print_document');
			...
			switch ($_GET['print_document']) {
				case 'society_notice':
					$account_model->addExpression('include')->set(function($m,$q){
						return $q->expr(
							'IF(
								[created_at]>="2018-04-01" AND [due_premium_count] = 2 AND [last_premium_field] >= "[last_premium_value_1_month]" AND  [paid_premium_count] <= [paid_premium_count_value] AND ([society_notice_sent_on] < "[today_minus_30_days]" OR [society_notice_sent_on] is null) AND [recent_due_premium_date] <= "[today_minus_15_days]" AND [bike_surrendered]=0 AND [is_given_for_legal_process]=0,
								1,
								IF(
									[created_at]<"2018-04-01" AND [due_premium_count] = 3 AND [last_premium_field] >= "[last_premium_value_3_month]" AND ([society_notice_sent_on] < "[today_minus_60_days]" OR [society_notice_sent_on] is null) AND  [bike_surrendered]=0 AND [is_given_for_legal_process]=0,
									1,
									0
								)
							)',
							[
								...
								'last_premium_value_1_month'=>$this->api->previousMonth($this->api->today. " -1 MONTH"),
								'last_paid_date_field'=>$m->getElement('last_paid_date'),
								'last_paid_date_value_1_month'=>$this->api->previousMonth($this->api->today. " -1 MONTH"),
								'paid_premium_count'=>$m->getElement('paid_premium_count'),
								'paid_premium_count_value'=>5,
								'society_notice_sent_on'=>$m->getElement('society_notice_sent_on'),
								'today_minus_30_days'=> date('Y-m-d',strtotime("-30 days",strtotime($this->app->today))),
								'today_minus_60_days'=> date('Y-m-d',strtotime("-60 days",strtotime($this->app->today))),
				...
				case 'legal_notice':
					$account_model->addExpression('include')->set(function($m,$q){
						return $q->expr(
							'IF(
								[created_at]>="2018-04-01" AND [due_premium_count] = 4 AND [last_premium_field] >= "[last_premium_value_3_month]" AND ([legal_notice_sent_on] < "[today_minus_120_days]" OR [legal_notice_sent_on] is null) AND [bike_surrendered]=0 AND [is_given_for_legal_process]=0,
								1,
								IF(
			...
			$account_model->addCondition('include',true);
		}else
		...
		if($form->isSubmitted()){
			...
			$send = array('dealer'=>$form['dealer'],'print_document'=>$form['print_document'], 'loan_type'=>$form['loan_type'],'filter'=>1);
			// foreach ($document as $junk) {
