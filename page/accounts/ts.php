<?php
class page_accounts_ts extends Page {
	public $title = "Testing Purpose Account Statement Only";
	...
	function init(){
		...
		$transactions->addExpression('tr_account')->set(function($m,$q){
			return $this->add('Model_Account',['table_alias'=>'tr_refacc'])->addCondition('id',$m->getField('reference_id'))->fieldQuery('agent');
		});

		$transactions->addExpression('tr')->set(function($m,$q){
			$tr  = $m->add('Model_Transaction',['table_alias'=>'tr_tab']);
			$tr->addCondition('id',$m->getElement('transaction_id'));
			// return $tr->fieldQuery('id');

			$tr_row = $m->add('Model_TransactionRow',['table_alias'=>'ag_tr_row'])->addCondition('transaction_id',$tr['id']);
			...
			return $tr_row->fieldQuery('account');
		});

		// $transactions->addExpression('agent')->set(function($m,$q){
		// 	return $this->add('Model_Account',['table_alias'=>'tr_refacc'])->addCondition('id',$m->getElement('tr'))->fieldQuery('agent');
		// });		
		...
		$grid->setModel($transactions,array('tr','voucher_no','created_at','invoice_no','Narration','amountDr','amountCr'));
