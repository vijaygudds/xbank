...
class page_roaccountassociation extends Page {
	...
	function init(){
		...
		// SECOND FORM FOR RO/MO LIST

		$ro_change_form= $this->add('Form');
		$ro_change_form->addField('DropDown','ro')->setEmptyText("Please Select")->validateNotNull()->setModel('Mo')->addCondition('is_active',true);
		$ro_change_form->addField('Checkbox','clear_ro','Clear above RO from existing accounts');
		$ro_list_field = $ro_change_form->addField('Text','account_list');
		$ro_change_form->addSubmit('GO');


		$grid->addSelectable($ro_list_field);

		if($ro_change_form->isSubmitted()){
			$account_list = json_decode($ro_change_form['account_list'],true);

			if(count($account_list)>0 && $ro_change_form['clear_ro']){
				$ro_change_form->displayError('clear_ro','Do not select accounts to clear, all account containing above selected Ro will be removed');
			}

			if($ro_change_form['clear_ro'] && !$ro_change_form['ro']){
				$ro_change_form->displayError('ro','Select Ro that you want to remove first');
			}
			...
			if($ro_change_form['clear_ro']){
				$account_m = $this->add('Model_Account_Loan');
				$account_m->addCondition('mo_id',$ro_change_form['ro']);
			...
			if(count($account_list)>0){	
				foreach ($account_list as $acc) {
					$account_m = $this->add('Model_Account_Loan');
					$account_m->load($acc);
					$account_m['mo_id']= $ro_change_form['ro'];
					$account_m->save();	
				}
			}
