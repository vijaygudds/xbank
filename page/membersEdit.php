...
class page_membersEdit extends Page {
	
	public $title='Member Edit';
	...
	function page_index(){
		...
		$crud = $this->add('xCRUD',array('grid_class'=>'Grid_Member','allow_add'=>false));
		$crud->addClass('membercrud');

		$member_model = $this->add('Model_MemberEdit');
		...
		$crud->addHook('myupdate',function($crud,$form){
			...
			$new_member_model = $crud->add('Model_MemberEdit');
			...
			$member_old = $this->add('Model_MemberEdit');
	...
	function isPanExists($value){
		return $this->add('Model_MemberEdit')->tryLoadBy('PanNo',$value)->count()->getOne();
	}
